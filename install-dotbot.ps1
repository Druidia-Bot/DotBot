...
.DESCRIPTION
    ...
    Usage:
      ...
      # or
      .\install-dotbot.ps1 -RepoUrl <url> [-Mode agent|server|both] [-ServerUrl wss://your.server:3001] [-InviteToken dbot-XXXX-...]
.PARAMETER Mode
...
.PARAMETER InstallDir
    Where to clone DotBot (default: ~/DotBot)
#>
...
function Get-InstallMode {
    ...
    Write-Host ""
    Write-Host "    [1] Local Agent  — connects to a hosted DotBot server  (most users)" -ForegroundColor White
    Write-Host "    [2] Server       — host your own DotBot server" -ForegroundColor White
    Write-Host "    [3] Both         — development / single-machine setup" -ForegroundColor White
    Write-Host ""
...
function Remove-UnneededPackages {
    ...
    elseif ($SelectedMode -eq "server") {
        ...
        foreach ($f in @("run.bat", "run.ps1", "run-dev.bat", "client.bat")) {
            $p = Join-Path $Dir $f
            if (Test-Path $p) { Remove-Item -Force $p; $deleted += $f }
        }
    }
...
if (-not $InstallDir) {
    $InstallDir = Join-Path $env:USERPROFILE "DotBot"
}
