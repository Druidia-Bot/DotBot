...
if (Test-Path $clientPath) {
    ...
    # Write client/config.js with connection details (most reliable for file:// protocol)
    $configJsPath = Join-Path $InstallDir "client\config.js"
    $configLines = @("// Auto-generated by installer -- do not edit manually")
    $configObj = "{"
    if ($serverWsUrl) {
        $escaped = $serverWsUrl -replace "'", "\'"
        $configObj += " wsUrl: '$escaped'"
    }
    $configObj += " }"
    $configLines += "window.DOTBOT_CONFIG = $configObj;"
    [System.IO.File]::WriteAllText($configJsPath, ($configLines -join "`n"), [System.Text.UTF8Encoding]::new($false))
