...
import { LocalLLMClient, isCloudReachable, isLocalModelReady, getLocalStatus } from "./local-llm.js";
import { selectModel, registerApiKeys } from "./model-selector.js";
...
describe("TIER_CONFIGS — local model in preferredModels", () => {
  it("all tiers reference 'local' not 'ollama'", () => {
    for (const tier of Object.values(TIER_CONFIGS)) {
      expect((tier.preferredModels as any).ollama).toBeUndefined();
      expect(tier.preferredModels.local).toBeDefined();
    }
  });
});
