...
export function selectModel(criteria: ModelSelectionCriteria): ModelSelection {
  ...
  // 5. Legacy tier mapping
  if (criteria.personaModelTier === "powerful") {
    return resolveRole(TIER_TO_ROLE["powerful"], "persona tier 'powerful' → architect");
  }

  // 6. Default → workhorse (DeepSeek V3.2)
...
/**
 * Fallback chain: if the preferred provider is unavailable, try alternatives.
 *
 * workhorse (DeepSeek) → Gemini Flash → OpenAI → local
 * deep_context (Gemini) → Anthropic (200K) → DeepSeek (64K, degraded)
 * architect (Anthropic) → DeepSeek Reasoner → Gemini Pro
 * local (node-llama-cpp) → DeepSeek (if online)
 */
function getFallback(role: ModelRole): { provider: LLMProvider; model: string; temperature: number; maxTokens: number } | null {
  const fallbackChains: Record<ModelRole, Array<{ provider: LLMProvider; model: string; temperature: number; maxTokens: number }>> = {
    workhorse: [
