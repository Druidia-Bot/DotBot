/**
 * Token Tracker — V2 Per-Device LLM Token Usage Recording
 *
 ...
 *
 * Wired via the onLLMResponse callback: after every LLM call,
 * the tracker logs model, role, token counts, and which agent used them.
 */
...
export interface TokenUsageEntry {
  deviceId: string;
  model: string;
  role: string;          // "receptionist", "agent", "judge", "supervisor", "reflector", "architect"
  inputTokens: number;
  outputTokens: number;
  agentId?: string;      // Which spawned agent used these tokens
}
...
/**
 * Get total tokens used by a specific agent (for reflector analysis).
 */
