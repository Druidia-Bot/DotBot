...
export async function runToolLoop(
  ...
  // Stuck detection: track consecutive calls to the same tool WITH identical args.
  // Calling the same tool with different args (e.g. search.brave with varied queries)
  // is normal research behavior, not being stuck.
  ...
  while (iterations < options.maxIterations) {
    ...
    // ── Stuck detection: same tool called repeatedly with identical args ──
    if (toolCalls.length === 1) {
      const currentToolId = unsanitizeToolName(toolCalls[0].function.name);
      const currentArgs = toolCalls[0].function.arguments;
      if (currentToolId === lastToolId && currentArgs === lastToolArgs) {
        consecutiveSameToolCount++;
      } else {
        consecutiveSameToolCount = 1;
        lastToolId = currentToolId;
        lastToolArgs = currentArgs;
      }
    ...
    if (consecutiveSameToolCount === STUCK_WARNING_THRESHOLD) {
      log.warn(`Stuck detection: ${lastToolId} called ${consecutiveSameToolCount} times with identical args — injecting warning`, { personaId });
    ...
    if (consecutiveSameToolCount >= STUCK_ESCALATE_THRESHOLD) {
      log.warn(`Stuck detection: ${lastToolId} called ${consecutiveSameToolCount} times with identical args — force-escalating`, { personaId });
      escalated = true;
