...
export async function runReceptionist(
  ...
  const serverPersonas = getServerPersonaSummaries();
  const personaTableSection = "\n\n" + formatPersonasTable(serverPersonas, request.userPersonas) + "\n";
  ...
  const personaToolMap = buildPersonaToolMap(serverPersonas, request.userPersonas);
...
Analyze the conversation and provide your routing decision as JSON.`;
  ...
  try {
    ...
    if (response.content.length > 2000) {
      ...
      // "ACTION") instead of a real response, don't use it as a direct response.
      const CLASSIFICATION_KEYWORDS = new Set([
        "CONVERSATIONAL", "ACTION", "INFO_REQUEST", "CONTINUATION",
        "CORRECTION", "COMPOUND", "GREETING", "ACKNOWLEDGMENT",
      ]);
      ...
      decision = {
        ...
        directResponse: isRawClassification ? undefined : (cleanedResponse || undefined),
