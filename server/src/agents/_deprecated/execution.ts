...
import { runToolLoop, buildExecutionHandlers, ESCALATE_TOOL_ID } from "../tool-loop/index.js";
import { manifestToNativeTools } from "./tools.js";
...
**NEVER say "I don't have that tool" or give up without first calling agent.request_tools.** Tool categories include: filesystem, shell, search, http, knowledge, research, discord, git, npm, data, database, pdf, vision, audio, and more.`;
  ...
  if (options.onExecuteCommand) {
    ...
    // Build handler map + synthetic tool definitions via composable layers
    const deviceId = options.deviceId || "";
    const { handlers, syntheticDefinitions } = buildExecutionHandlers(
      filteredManifest || [],
      {
        workspacePath: undefined, // workspace path set via saveToWorkspace callback
        syntheticConfig: {
          waitForUser: !!onWaitForUser,
          requestTools: true,
          requestResearch: !!options.onExecuteResearchTool,
        },
      },
    );

    // Convert manifest to native tool definitions + merge synthetic tools
    const nativeTools = [
    ...
    const messages = buildMessagesWithHistory(systemPrompt, userMessage, request);

    // Wire server-side executor callbacks into ctx.state
    const ctxState: Record<string, any> = {};
    if (options.onExecutePremiumTool) {
      ctxState.executePremiumTool = options.onExecutePremiumTool;
    }
    if (options.onExecuteImageGenTool) {
      // Curry executeCommand into the imagegen executor
      ctxState.executeImageGenTool = (toolId: string, args: Record<string, any>) =>
        options.onExecuteImageGenTool!(toolId, args, options.onExecuteCommand!);
    }
    if (options.onExecuteKnowledgeIngest) {
      ctxState.executeKnowledgeIngest = options.onExecuteKnowledgeIngest;
    }
    if (options.onExecuteScheduleTool) {
      ctxState.executeScheduleTool = options.onExecuteScheduleTool;
    }
    if (options.onExecuteResearchTool) {
      // Curry executeCommand into the research executor
      ctxState.executeResearchTool = (toolId: string, args: Record<string, any>) =>
        options.onExecuteResearchTool!(toolId, args, options.onExecuteCommand!);
    }
    if (onWaitForUser) {
      ctxState.onWaitForUser = onWaitForUser;
    }
    ...
    const result = await runToolLoop({
      ...
      handlers,
      maxIterations: 50,
      context: { deviceId, state: ctxState },
      ...
      onToolResult: (tool: string, result: string, success: boolean) => {
    ...
    return {
      ...
      escalated: result.escalated ? true : undefined,
