...
import { sendMemoryRequest } from "../bridge/commands.js";
import { sendRunLog, sendSaveToThread } from "../bridge/notifications.js";
import { prepareDot } from "../../dot/pre-dot/prepare.js";
...
export async function handlePrompt(
  ...
  try {
    ...
    const dotOpts = { llm, userId, deviceId, prompt, messageId: message.id, source };

    // Phase 1: Pre-dot — tailor principles, resolve prompt
    const prepared = await prepareDot(dotOpts);

    // Save resolved user message to thread before Dot runs
    sendSaveToThread(userId, prepared.threadId, {
      role: "user",
      content: prepared.resolvedPrompt,
      source,
      messageId: message.id,
    }, "Conversation");

    // Phase 2: Run Dot's tool loop
    const dotResult = await runDot(dotOpts, prepared);
    ...
    // Save raw user message so the thread records the user spoke even on error
